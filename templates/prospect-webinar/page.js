...
(function () {
  ...
  var FALLBACK = {
    ...
    company: 'Acme Corp', industry: 'SaaS', current_platform: '',
    observation: '',
  ...
  /* If current_platform is filled, tweak hero sub-text to name it */
  var platform = (V.current_platform || '').trim();
  if (platform) {
    var subEl = document.getElementById('hero-sub-text');
    if (subEl) {
      subEl.textContent = 'You\'re already running webinars on ' + platform + ' — that\'s a real foundation. The teams getting the most from their events programs figured out something important: the live session is just the beginning. What happens after — replays, repurposed content, follow-up sequences — is where the real value compounds.';
    }
  }
